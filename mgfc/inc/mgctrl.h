...
protected:
	...
	inline int CallOldProc(int iMsg,WPARAM wParam,LPARAM lParam)
		{ if(_oldProc)(*_oldProc)(MGSUBCLASS_HWND,iMsg,wParam,lParam); }
...
protected:
	...
	virtual BOOL WndProc(int iMsg,WPARAM wParam,LPARAM lParam,int *pret)=0;
	inline int CallOldProc(int iMsg,WPARAM wParam,LPARAM lParam)
		{ if(m_oldProc)(*m_oldProc)(MGSUBCLASS_HWND,iMsg,wParam,lParam); }
private:
	static int _newCtrlProc(HWND hWnd,int iMsg,WPARAM wParam,LPARAM lParam)
	{
...
{
public:
	MGCtrlWnd(HWND hWnd):MGWnd(hWnd){}
...
public:
	...
	inline int ReplaceAll(const char* pFind,const char* pReplace,BOOL fCaps=FALSE)
	{
		SetCaretPos(0,0);
...
public:
	...
	inline GHANDLE AddItem(GHANDLE parent,const char* text,DWORD dwFlags,HICON hIconFold=(HICON)NULL,HICON hIconUnfold=(HICON)NULL,DWORD dwAddData=0)
	{
		TVITEMINFO tvItem ;
			tvItem.text = (char*)text;
			tvItem.dwFlags = dwFlags;
			tvItem.hIconFold = hIconFold;
			tvItem.hIconUnfold = hIconUnfold;
	...
	inline HICON GetItemIconFold(GHANDLE item)
	{
		TVITEMINFO tvii;
		if(GetItemInfo(item,&tvii)==0)
		{
			return tvii.hIconFold;
		}
		return (HICON)NULL;
	}

	inline HICON GetItemIconUnfold(GHANDLE item)
	{
		TVITEMINFO tvii;
		if(GetItemInfo(item,&tvii)==0)
		{
			return tvii.hIconUnfold;
		}
		return (HICON)NULL;
	}
	...
	inline int SetItemInfo(GHANDLE item,const char* text,HICON hIconFold=(HICON)NULL,HICON hIconUnfold=(HICON)NULL,DWORD dwFlags=0,DWORD dwAddData=0)
	{
		TVITEMINFO tvii;
		tvii.text = (char*)text;
		tvii.hIconFold = hIconFold;
		tvii.hIconUnfold = hIconUnfold;
	...
	inline int SetItemIcon(GHANDLE item,HICON hIconFold,HICON hIconUnfold)
	{
		TVITEMINFO tvii;
		tvii.text = NULL;
		if(GetItemInfo(item,&tvii))
